...
class Login_attempts extends CI_Model
{
	...
	function get_attempts_num($ip_address, $login)
	{
		...
		if (strlen($login) > 0) $this->db->or_where('login', strtolower($login));
	...
	{
		$this->db->insert($this->table_name, array('ip_address' => $ip_address, 'login' => strtolower($login)));
	}

	function clear_attempts($ip_address, $login, $expire_period = 86400)
	{
		$this->db->where(array('ip_address' => $ip_address, 'login' => strtolower($login)));
